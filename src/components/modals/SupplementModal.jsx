...
const SupplementModal = ({ contratoSeleccionado,tipoContrato }) => {
  const [selectedContractForSupplement, setSelectedContractForSupplement] = useState(contratoSeleccionado);
  ...
  const { showSupplementModal, setShowSupplementModal,obtenerRegistros,setIsCreate } = useValidation();
  ...
  const handleAddSupplement = async() => {
    ...
    try {
      const { data } = await clienteAxios.post(
        `/contratos/suplementos/${selectedContractForSupplement?._id}`, supplementData,
        config
      );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative w-full max-w-md p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl animate-slideIn">
            ...
            <h3 className="text-lg font-semibold mb-4 dark:text-gray-200">
              Agregar Suplemento al contrato N.Dictamen={">"} # {selectedContractForSupplement?.numeroDictamen || '#'}
            </h3>
            <div className="mb-4">
