...
const SupplementModalEdit = ({ suplemento,tipoContrato,contractName, contractId,setActualSupplement }) => {
  const [supplementTypes, setSupplementTypes] = useState({
    tiempo:suplemento?.tiempo.days!==0 || suplemento?.tiempo.months!==0 || suplemento?.tiempo.years!==0 ? true :false ,
  ...
  const [amountSupplement, setAmountSupplement] = useState(suplemento.monto || '');
  const [supplementName, setSupplementName] = useState(suplemento.nombre || '');
  const { showSupplementModalEdit, setShowSupplementModalEdit,obtenerRegistros, handleGetSupplements,setIsCreate } = useValidation();
  ...
  const handleAddSupplement = async() => {
    ...
    if (supplementTypes.tiempo) {
      if (!timeSupplement.days && !timeSupplement.months && !timeSupplement.years) {
    ...
    try {
      const { data } = await clienteAxios.put(
        `/contratos/suplementos/${suplemento._id}`, supplementData,
        config
      );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative w-full max-w-md p-6 bg-white dark:bg-gray-800 rounded-lg shadow-xl animate-slideIn">
            ...
            <h3 className="text-lg font-semibold mb-4 dark:text-gray-200">
              Actualizar suplemento del contrato N.Dictamen={">"} # {contractName || '#'}
            </h3>
            <div className="mb-4">
        ...
        />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <button
                  ...
                  className={`p-3 rounded-lg border-2 flex items-center justify-center transition-all
                    ...
                    dark:text-gray-200`}
                >
                  <FaMoneyBillAlt className="mr-2" /> Monto
                </button>
              </div>
            </div>
              ...
              <div className="mb-4">
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="relative">
                    ...
                    />
                    <span className="absolute right-2 top-2.5 text-sm text-gray-500 dark:text-gray-400">D</span>
                  </div>
                  <div className="relative">
                    <input
                      type="number"
                      ...
                      className="w-full p-2 border rounded dark:bg-gray-700 pr-8 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600"
                    />
                    <span className="absolute right-2 top-2.5 text-sm text-gray-500 dark:text-gray-400">M</span>
                  </div>
                  <div className="relative">
                    <input
                      type="number"
                      ...
                      className="w-full p-2 border rounded dark:bg-gray-700 pr-8 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600"
                    />
                    <span className="absolute right-2 top-2.5 text-sm text-gray-500 dark:text-gray-400">A</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {supplementTypes.monto && (
              <div className="mb-4">
                ...
                <div className="relative">
                  <span className="absolute left-3 top-3 dark:text-gray-300">$</span>
                  <input
                    type="number"
                    min="0"
                    step="0.01"
                    ...
                    onChange={(e) => setAmountSupplement(e.target.value.replace(/[^0-9.]/g, ''))}
