...
import { FaBriefcase, FaHandshake, FaHome, FaLaptop, FaUserTie, FaEdit, FaTrash, FaPlus, FaFileContract, FaBalanceScale, FaBuilding, FaChartLine, FaClipboardList, FaCogs, FaCoins, FaFileSignature, FaGlobe, FaIndustry, FaLandmark, FaMoneyBillAlt, FaRegHandshake, FaSuitcase, FaTools } from "react-icons/fa";
...
const ContractConfigPanel = () => {
  ...
  const requiredFields = [
    { id: "objetoContrato", label: "Objeto del Contrato", tooltip: "Descripción del objeto del contrato" },
    { id: "entidad", label: "Entidad", tooltip: "Entidad involucrada en el contrato" },
    { id: "direccionEjecutiva", label: "Dirección Ejecutiva", tooltip: "Dirección ejecutiva responsable" },
    { id: "aprobadorCC", label: "Aprobador por el CC", tooltip: "Fecha que fue aprobada por el Comité de Contrataciones" },
    { id: "fechaFirmada", label: "Fecha Firmada", tooltip: "Fecha en que se firmó el contrato" },
    { id: "entregadoJuridica", label: "Entregado Jurídica", tooltip: "Fecha que fue entregado al área jurídica" },
    { id: "fechaRecibido", label: "Fecha Recibido", tooltip: "Fecha de recepción del contrato" },
    ...
    { id: "numeroDictamen", label: "Número de Dictamen", tooltip: "Número de dictamen asociado" },
    { id: "subirPDF", label: "Subir PDF", tooltip: "Subir archivo PDF del contrato" }
  ];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await clienteAxios.put(
        url,
        {
          camposRequeridos: requiredFields.filter(field => selectedFields.includes(field.id)).map(field => ({
            id: field.id,
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white rounded-lg shadow-md p-6 border-t-4 border-blue-500">
            <h2 className="text-2xl font-semibold mb-6 text-gray-800">Tipos de Contrato</h2>
            <div className="grid gap-4">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                      <div className={`text-2xl ${selectedType?._id === type._id ? "text-blue-600" : "text-gray-600"}`}>
                        {availableIcons.find(icon => icon.name === type.icon)?.icon()}
                      </div>
                      <div>
                        <h3 className="font-medium text-gray-800">{type.nombre}</h3>
                    ...
                    <div className="flex space-x-2">
                      <button
                        onClick={(e) => {
                          ...
                          setSelectedIcon(availableIcons.find(icon => icon.name === type.icon));
              ...
              <button
                ...
                className="flex items-center justify-center p-4 rounded-lg border-2 border-dashed border-gray-300 hover:border-blue-500 hover:bg-blue-50 transition-all"
              >
                <FaPlus className="text-gray-500 mr-2" />
                <span className="text-gray-700">Agregar Tipo de Contrato</span>
              </button>
            </div>
          </div>

          <div className="bg-white rounded-lg shadow-md p-6 border-t-4 border-indigo-500">
            <h2 className="text-2xl font-semibold mb-6 text-gray-800">Campos Requeridos</h2>
            {!selectedType ? (
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  <h3 className="font-medium text-gray-700 border-b pb-2">Información del Contrato</h3>
                  <div className="space-y-4">
                    {requiredFields.map((field) => (
                      <div key={field.id} className="flex items-center justify-between p-2 rounded transition-colors mb-4">
                        <div className="flex items-center space-x-3">
                          ...
                          />
                          <label htmlFor={`${selectedType._id}-${field.id}`} className="text-gray-700">{field.label}</label>
                        </div>
                        <div
                          className="relative"
                        ...
                        >
                          ...
                          {hoveredField === field.id && (
                            <span className="absolute right-0 top-6 w-48 p-2 bg-gray-800 text-white text-sm rounded-md z-10">
                              {field.tooltip}
                            </span>
                          )}
                        </div>
      ...
      {showModalCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative w-full max-w-md p-6 bg-white rounded-lg shadow-xl animate-slideIn">
            ...
            <h3 className="text-lg font-semibold mb-4">Crear Tipo de Contrato</h3>
            <div className="mb-4">
              <label htmlFor="contractName" className="block text-gray-700 text-sm font-semibold mb-1">
                Tipo de Contrato
              </label>
              <input
                type="text"
            ...
            <div className="mb-4">
              <label htmlFor="contractDescription" className="block text-gray-700 text-sm font-semibold mb-1">
                Descripción
              </label>
              <input
                type="text"
      ...
      {showModalUpdate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="relative w-full max-w-md p-6 bg-white rounded-lg shadow-xl animate-slideIn">
            ...
            <h3 className="text-lg font-semibold mb-4">Editar Tipo de Contrato</h3>
            <div className="mb-4">
              <label htmlFor="contractName" className="block text-gray-700 text-sm font-semibold mb-1">
                Tipo de Contrato
              </label>
...
{showModalDelete && (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div className="relative w-full max-w-md p-6 bg-white rounded-lg shadow-xl animate-slideIn">
      ...
      <h3 className="text-lg font-semibold mb-4">Eliminar Tipo de Contrato</h3>
      <p className="text-gray-700 mb-4">¿Estás seguro de que deseas eliminar este tipo de contrato?</p>

      {/* Advertencia de contratos asociados */}
      {contractAsociation > 0 && (
        <div className="mb-4">
          <p className="text-red-600 font-medium">
            Advertencia: Este tipo de contrato tiene {contractAsociation} contrato(s) asociado(s).
          </p>
          <p className="text-gray-700 mt-2">
            Si deseas eliminar este tipo de contrato, primero exporta los registros asociados.
          </p>
          <button
...
{contractAsociation > 0 && (
  <div className="flex items-center mb-4 justify-start">
    ...
    <label htmlFor="confirmDelete" className="text-gray-700">
      Confirmo que deseo eliminar este tipo de contrato y soy consciente de los riesgos.
    </label>
  </div>
)}
